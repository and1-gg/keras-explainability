...
1.1 Scatter Plot der Prädizierten vs wahren Rechter-Thalamus-Volumina, um zu sehen ob die Predictions überhaupt Sinn machen, also man versucht ein "funktionierendes" model zu erklären
1.2 genommen wurden 10 input subjects aus den Hold-out datensätzen von IXI und UKB
1.3 3d intensitätsplot der eines trainingssubjects UKB um zu sehen wie groß die typschen und min/max intensitätswerte sind
1.4 ein histogram plot eines trainingssubjects von UKB um die häufigkeit der intensitätswerte zu sehen
2. **Sanity Checks XAI-LRP:**
2.1 Heatmaps erzeugt für die ersten 2 Subjects aus dem Hold-out datensatz für UKB/IXI mit nicht normierten Relevanzen, linker und rechter thalamus sind einhgezeichnet für optische kontrolle ob sich die posistiv + stark prädiktiven Voxel (rot) im rechten thalamus befinden. (rot = treibt die
Vorhersage nach oben, blau = zieht sie nach unten). Alle 2D-Overlays nutzen dieselben festen Schnitte — sagittal `x = 70`, koronal `y = 104`, axial `z = 78`.
2.2 Tabelle mit subject-id, wahrem wert, prediction für rechten thalamus und summer aller relevanzen der heatmap, um zu testen ob die LRP erhaltung gewährleistet ist, dass ja bei dem Regressions model der geschätzte volumen wert rückwährts über alle layer auf die input voxel verteilt wird und relevanz erhaltung gewährleistet sein muss.
2.3 Interaktiver 3d plot der heatmaps und thalamus volumina für das erste subject aus den hold-out datensätzen für UKB/IXI um optisch reinzoomen zu können und zu besser zu sehen, ob die relevanzen einfach nur diffus verteilt sind oder irgendwie anatomisch geclustert 


### **Teil B: CNN rechter Thalamus Model trainiert auf UKB Daten - alles gejittert außer rechter Thalamus**
* **Sanity Checks Training:** 
  * Scatter Plot der Prädizierten vs wahren Rechter-Thalamus-Volumina, um zu sehen ob die Predictions überhaupt Sinn machen, also man versucht ein "funktionierendes" model zu erklären
  * genommen wurden 10 input subjects aus den Hold-out datensätzen von IXI und UKB
  * 3d intensitätsplot der eines trainingssubjects UKB um zu sehen wie groß die typschen und min/max intensitätswerte sind
  * ein histogram plot eines trainingssubjects von UKB um die häufigkeit der intensitätswerte zu sehen
  * querschnittsplots von 3 UKB trainingssubjects, um visuell zu testen ob wirklich nur der rechte thalamus gejittert ist und um zu sehen in welchen bereich die intensitätswerte liegen, oder ob es ggf aureißer gab durch jittern
* **Sanity Checks XAI-LRP:**
  * Heatmaps erzeugt für die ersten 2 Subjects aus dem Hold-out datensatz für UKB/IXI mit nicht normierten Relevanzen, linker und rechter thalamus sind einhgezeichnet für optische kontrolle ob sich die posistiv + stark prädiktiven Voxel (rot) im rechten thalamus befinden. (rot = treibt die
  Vorhersage nach oben, blau = zieht sie nach unten). Alle 2D-Overlays nutzen dieselben festen Schnitte — sagittal `x = 70`, koronal `y = 104`, axial `z = 78`.
  * Tabelle mit subject-id, wahrem wert, prediction für rechten thalamus und summer aller relevanzen der heatmap, um zu testen ob die LRP erhaltung gewährleistet ist, dass ja bei dem Regressions model der geschätzte volumen wert rückwährts über alle layer auf die input voxel verteilt wird und relevanz erhaltung gewährleistet sein muss.
  * Interaktiver 3d plot der heatmaps und thalamus volumina für das erste subject aus den hold-out datensätzen für UKB/IXI um optisch reinzoomen zu können und zu besser zu sehen, ob die relevanzen einfach nur diffus verteilt sind oder irgendwie anatomisch geclustert 
