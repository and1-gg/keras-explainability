...
Bericht zum Notebook `notebooks/ipynb_files/analysis_LRP_for_right_thalamus_volume_based_on_CNN_prediction.ipynb`
...
Ein 3D-CNN (Architektur `sfcn-reg`) wurde darauf trainiert, aus einem T1-gewichteten
MRT-Volumen das **Volumen des rechten Thalamus** (`Right-Whole_thalamus`, in mm³)
vorherzusagen. Das Modell erreicht auf Holdout-Daten eine sehr hohe Genauigkeit.

Eine hohe Genauigkeit allein sagt aber nichts darüber aus, **woher** das Modell seine
Vorhersage bezieht. Ein Netz könnte das Thalamus-Volumen auch indirekt schätzen — etwa
über die globale Kopf- oder Gehirngröße, über die Ventrikelweite oder über andere
Strukturen, die mit dem Thalamus-Volumen korrelieren. Für eine neurowissenschaftliche
Interpretation oder einen klinischen Einsatz wäre das ein Problem: Das Modell wäre dann
kein Thalamus-Messgerät, sondern ein Korrelations-Schätzer.

**Leitfrage des Notebooks:**

> Leitet das Modell seine Vorhersage tatsächlich aus dem rechten Thalamus ab
> oder aus dem umgebenden Gewebe?

### 1.2 Untersuchungsstrategie

Die Frage wird mit drei unabhängigen, sich gegenseitig absichernden Ansätzen angegangen.
Diese Dreiteilung ist der Aufbau des gesamten Berichts:

| Teil | Ansatz | Leitfrage |
|---|---|---|
| **A** | **Korrelativ** — Layer-wise Relevance Propagation (LRP) auf unmanipulierten Volumes, Relevanz gegen anatomische FreeSurfer-Masken verrechnet | *Wo* liegt die Relevanz? |
| **B** | **Kausal** — gezielte Manipulation des Inputs (Thalamus erhalten, Rest permutiert) und Beobachtung der Vorhersage-Reaktion | Ändert sich die Vorhersage, wenn nur das Umfeld zerstört wird? |
| **C** | **Methodisch** — Prüfung der Relevanzerhaltung Schicht für Schicht | Ist die LRP-Messung selbst überhaupt vertrauenswürdig? |

Teil A ist nur ein Indiz: LRP zeigt, wo Relevanz *zugewiesen* wird, nicht ob diese
Region *kausal notwendig* ist. Teil B liefert deshalb den kausalen Gegentest. Teil C ist
ein Kontrollschritt, ohne den die Ergebnisse aus A und B nicht belastbar wären — würde
die LRP-Implementierung Relevanzmasse verlieren oder erzeugen, wären alle Prozentangaben
aus Teil A bedeutungslos.

### 1.3 Datenbasis und Modelle
...
| **Original-Modell** | `training_run_21h19m18s_20aug2026` (auf unmanipulierten Volumes trainiert) |
| **Jitter-Modell** | `training_run_05h09m52s_04sep2026` (auf gejitterten Volumes trainiert) |
| Datensätze | **IXI** (n = 5) und **UKB**-Holdout aus dem offiziellen predict-Split (n = 5) |
| Laufzeitumgebung | TensorFlow 2.21.0, 2× NVIDIA RTX A6000 |

> **Einschränkung, die für den ganzen Bericht gilt:** Es wurden nur **5 Fälle pro
> Datensatz** ausgewertet (`N_SUBJECTS = 5`). Alle Kennzahlen sind damit
> Plausibilitätsprüfungen, keine statistisch belastbaren Effektschätzungen.
...
## 2. Teil A — Original-Modell auf unmanipulierten Volumes

**Ziel dieses Teils:** Zunächst die Vorhersagequalität des Modells bestätigen und dann
per LRP quantifizieren, welcher Anteil der Relevanz anatomisch im rechten Thalamus
liegt. Das ist der korrelative Zugang zur Leitfrage.

### 2.1 Vorhersagequalität (Abschnitt A.8)
...
Das deutet auf einen Domain-Shift zwischen den Datensätzen hin (Scanner, Sequenz,
Vorverarbeitung). Die MAE-Differenz ist der Bezugsmaßstab für Teil B.

### 2.2 LRP-Heatmaps (Abschnitt A.7)
...
### 2.3 Relevanzanteil im Thalamus (Abschnitt A.9)

**Ziel:** Den visuellen Eindruck aus A.7 in eine Zahl übersetzen. Gemessen wird der
...
**Ziel:** Die 2D-Schnitte aus A.7 zeigen nur drei Ebenen und können die räumliche
Beziehung zwischen Relevanzwolke und Thalamus-Masken verzerren. Ein frei drehbarer
3D-Plot macht überprüfbar, ob die Relevanz den Thalamus tatsächlich umschließt.
...
Die Dateien sind interaktiv (drehen, zoomen, verschieben) und lassen sich daher nicht als
