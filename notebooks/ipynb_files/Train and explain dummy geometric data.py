...
def get_label(y, i, encoder=None):
    """String-Label aus y holen — vor oder nach OneHotEncoder."""
    label = y[i]
    if isinstance(label, (str, np.str_)):
        return str(label)
    if encoder is not None:
        return encoder.categories_[0][np.argmax(label)]
    # Fallback, falls encoder noch nicht existiert
    return ["square", "circle", "noise"][np.argmax(label)]
...
def plot_volume_3d(
    ...
    if not np.any(occupied):
        print(f"{title}: keine Voxel über threshold={threshold}")
        return
    ...
    if len(coords) > max_points:
        ...
        coords = coords[idx]
        values = vol[occupied][idx]
    else:
        values = vol[occupied]

    if class_name in ("square", "circle"):
...
# Variante A: encoder existiert schon (nach OneHotEncoder)
for i in range(10):
    class_name = get_label(y, i, encoder=encoder)
    plot_volume_3d(
        X[i],
        title=f"Sample {i}: {class_name}",
        threshold=0.5 if class_name != "noise" else 0.6,
